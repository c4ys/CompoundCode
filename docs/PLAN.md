...
**暂不包含**：
- `/compoundcode plan` - 用户可以手动创建 plan.md
...
### 阶段 3：compoundcode-init skill（M3）
...
### 阶段 4：compoundcode-new skill（M4）

**目标**：实现 `/compoundcode new` 命令
...
| 里程碑 | 目标 | 交付物 |
|--------|------|--------|
| M1 | 插件基础 | plugin.json, 目录结构 |
| M2 | 文档模板 | 所有文档模板设计 |
| M3 | init skill | compoundcode-init.md 可用 |
| M4 | new skill | compoundcode-new.md 可用 |
| M5 | 工作流 skill | compoundcode-workflow.md |
| M6 | MVP 发布 | marketplace 发布 |

---

## 下一步行动

1. **立即开始**：创建插件基础结构（M1）- plugin.json 和 skills 目录
2. **优先实现**：`compoundcode-init` skill（M3），这是核心功能
...
- `/compoundcode plan` - 生成实施计划的 skill
